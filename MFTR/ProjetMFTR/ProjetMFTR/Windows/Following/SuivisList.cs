...
namespace ProjetMFTR
{
	public partial class SuivisList : Form
	{
		...
		/// </summary>
		public SuivisList(List<Entities.Suivi> suivis)
		{
			InitializeComponent();
		...
		private void Init()
		{
			Init(Connexion.Instance().Suivi.OrderByDescending((x) => x.dateSuivi).ThenByDescending(p => p.suivi_id).ToList());
		}

		/// <summary>
		/// Initialisation de la grille des suivis
		/// </summary>
		private void Init(List<Entities.Suivi> suivis)
		{
			//Connexion.connexionActionsSuivi.ObjectContextUpdater();
